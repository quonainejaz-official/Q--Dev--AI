...
1. **Wire real token streaming** — stream from OpenCode Zen (SSE/chunked) instead of buffering then fake-typing. Lower latency, better UX. (`opencodeService.js`, `chatController.js`)
2. **Add tests** — controllers, auth flow, chats CRUD/migration, image gen. CI already runs `npm test` on Node 18/20/22. (`tests/`)
